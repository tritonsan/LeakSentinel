...
OUT = Path("data/thermal/zone-1"); OUT.mkdir(parents=True, exist_ok=True)
...
def make_frame(has_leak: bool, seed: int) -> Image.Image:
    ...
    if has_leak:
        x0 = int(rng.integers(120, w-120)); y0 = py + int(rng.integers(-40,40))
...
def main():
    ...
    print("Wrote thermal frames to", OUT)
